...
namespace UniversityApiBackend
{
    public class Service
    {
        ...
        public IEnumerable<Student> FindOlderStudents()
        {
            ...
            var olderStudent = from student in students
                               where student.Dob < DateTime.Now.AddYears(-18)
                               select student;
            return olderStudent;
        }

        public IEnumerable<Student> GetAllStudentWithOneOrMoreCouses()
        {
            var students = _context.Students;

            var studentWithAtLeastOneCourse = from student in students
                                              where student.Courses.Count() > 0
                                              select student;
            return studentWithAtLeastOneCourse;
        }
